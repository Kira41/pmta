    ...
    try:
        ...
        class Path(object):
            ...
            def __init__(self, value):
                self._path = os.path.abspath(str(value))
            ...
            def iterdir(self):
                for name in os.listdir(self._path):
                    yield Path(os.path.join(self._path, name))
            ...
            def open(self, mode='r', encoding=None, errors=None):
                if encoding or errors:
                    import io
                    return io.open(self._path, mode, encoding=encoding, errors=errors)
...
try:
    ...
    class HTTPException(Exception):
        ...
        def __init__(self, status_code=500, detail=None):
            Exception.__init__(self, str(detail or 'HTTP error'))
    ...
    class JSONResponse(dict):
        ...
        def __init__(self, status_code=200, content=None):
            dict.__init__(self, status_code=status_code, content=content or {})
...
ALLOWED_KINDS = {'acct': ['acct-*.csv'], 'diag': ['diag-*.csv'], 'log': ['log', 'log.*', 'pmtahttp.log', 'pmtahttp.log.*'], 'pmtahttp': ['pmtahttp.log', 'pmtahttp.log.*'], 'all': ['acct-*.csv', 'diag-*.csv', 'log', 'log.*', 'pmtahttp.log', 'pmtahttp.log.*']}
_TAIL_STATE_LOCK = threading.Lock()
_TAIL_STATE = {}
_CSV_HEADER_STATE_LOCK = threading.Lock()
_CSV_HEADER_STATE = {}
_BRIDGE_STATUS_LOCK = threading.Lock()
_BRIDGE_STATUS = {'last_processed_file': '', 'last_cursor': '', 'parsed': 0, 'skipped': 0, 'unknown_outcome': 0, 'last_error': ''}
...
def _error_payload(error, detail=None):
    return {'ok': False, 'error': str(error or 'request_failed'), 'detail': detail or {}}
...
def request_validation_exception_handler(_, exc):
    return JSONResponse(status_code=400, content=_error_payload(error='invalid_parameters', detail={'message': 'Invalid request parameters', 'issues': exc.errors()}))

@app.exception_handler(Exception)
def unhandled_exception_handler(_, exc):
    return JSONResponse(status_code=500, content=_error_payload(error='internal_error', detail={'message': str(exc)}))
ACCOUNTING_HEADER_CANDIDATES = ['x-job-id', 'x-campaign-id', 'job-id', 'campaign-id', 'job_id', 'campaign_id', 'msgid', 'message-id', 'message_id']
_PMTA_MESSAGE_ID_RE = re.compile('^<[^.<>@\\s]+[.]([^.<>@\\s]+)[.]([^.<>@\\s]+)[.]c\\d+[.]w\\d+@[^>]+>$', re.IGNORECASE)
_JOBID_RE_1 = re.compile('[.][a-f0-9]{8,64}[.]([a-f0-9]{12})[.]([a-f0-9]{8,64}|none)[.]c[0-9]+[.]w[0-9]+@local', re.IGNORECASE)
_JOBID_RE_2 = re.compile('[.][a-f0-9]{8,64}[.]([a-f0-9]{12})[.]c[0-9]+[.]w[0-9]+@local', re.IGNORECASE)
...
def _parse_ids_from_message_id(value):
...
def _normalize_job_id(value):
    ...
    if not raw:
        return ''
    if re.fullmatch('[a-f0-9]{12}', raw):
        return raw
    from_text = _extract_job_id_from_text(raw)
    if from_text:
        return from_text
    m = re.search('\\b([a-f0-9]{12})\\b', raw)
    if m:
        return str(m.group(1) or '').strip().lower()
    return raw

def _normalize_outcome_type(v):
...
def _walk_accounting_events(patterns):
...
def _file_matches(name, patterns):
    return any((fnmatch.fnmatch(name, pat) for pat in patterns))

def list_dir_files(patterns):
    if not PMTA_LOG_DIR.is_dir():
        raise HTTPException(status_code=500, detail='Directory not found: {}'.format(PMTA_LOG_DIR))
    items = []
    for p in PMTA_LOG_DIR.iterdir():
        try:
            ...
            st = p.stat()
            mtime = st.st_mtime
            items.append({'name': name, 'size_bytes': int(st.st_size), 'mtime_epoch': int(mtime), 'mtime_utc': _utc_iso_from_epoch(mtime), 'mtime_local': datetime.fromtimestamp(mtime).isoformat()})
        except FileNotFoundError:
            continue
    return items

def _find_matching_files(patterns):
    candidates = []
    for p in PMTA_LOG_DIR.iterdir():
...
def _find_latest_file(patterns):
    candidates = []
    for p in PMTA_LOG_DIR.iterdir():
        if not p.is_file() or p.is_symlink():
            continue
...
def _read_new_lines(path, max_lines):
    ...
    return {'file': path.name, 'from_offset': start_off, 'to_offset': next_off, 'count': len(lines), 'has_more': has_more, 'lines': lines}

def _parse_accounting_line(line, source_file='', line_no_or_offset=''):
    ...
    if fields and any((x.lower() in {'type', 'event', 'rcpt', 'recipient', 'msgid', 'message-id', 'message_id'} for x in fields)):
        ...
        return None
    ev = {'raw': s, 'source_file': source_file, 'line_no_or_offset': line_no_or_offset}
    with _CSV_HEADER_STATE_LOCK:
...
def _structured_event(ev):
    message_id = _event_value(ev, 'header_message-id', 'message-id', 'message_id', 'msgid', 'messageid')
    return {'type': str(_event_value(ev, 'type') or '').strip().lower(), 'outcome': _normalized_outcome(ev), 'time_logged': _event_value(ev, 'timeLogged', 'timelogged', 'time_logged', 'time'), 'orig': _event_value(ev, 'orig', 'mailfrom', 'from'), 'rcpt': _event_value(ev, 'rcpt', 'recipient', 'email', 'to', 'rcpt_to'), 'job_id': _event_job_id(ev), 'campaign_id': _event_campaign_id(ev), 'message_id': message_id, 'dsn_action': _event_value(ev, 'dsnAction', 'dsn_action'), 'dsn_status': _event_value(ev, 'dsnStatus', 'dsn_status'), 'dsn_diag': _event_value(ev, 'dsnDiag', 'dsn_diag'), 'vmta': _event_value(ev, 'vmta'), 'dlv_source_ip': _event_value(ev, 'dlvSourceIp', 'dlv_source_ip'), 'dlv_dest_ip': _event_value(ev, 'dlvDestinationIp', 'dlv_dest_ip', 'dlvDestinationIP'), 'bounce_cat': _event_value(ev, 'bounceCat', 'bounce_cat'), 'source_file': str(ev.get('source_file') or ''), 'line_no_or_offset': ev.get('line_no_or_offset', '')}

def _event_header_value(ev):
    normalized = {}
    for k, v in (ev or {}).items():
...
def _group_accounting_events(lines, source_file):
    events = []
    groups = {}
    for line in lines:
        ...
        if grp_key not in groups:
            groups[grp_key] = {'header_key': header_key, 'header_value': header_value, 'count': 0, 'emails': [], 'events': []}
    ...
    batches = sorted(groups.values(), key=lambda g: (g.get('header_key') != 'unknown', int(g.get('count') or 0)), reverse=True)
    return {'events': events, 'batches': batches}
...
def _merge_batches(batches):
    ...
    for batch in batches:
        ...
        if key not in merged:
            merged[key] = {'header_key': key[0], 'header_value': key[1], 'count': 0, 'emails': [], 'events': []}
        dst = merged[key]
    ...
    return sorted(merged.values(), key=lambda g: (g.get('header_key') != 'unknown', int(g.get('count') or 0)), reverse=True)

def _normalize_match_value(v):
    s = str(v or '').strip().lower()
    if not s:
        return ''
    if s.startswith('<') and s.endswith('>'):
...
def _event_matches_filter(ev, filters):
    if not filters:
        return True
    normalized = {}
    for k, v in (ev or {}).items():
...
def _build_batches_from_events(events):
    by_key = {}
    for ev in events or []:
        header_key, header_value = _event_header_value(ev)
        key = (header_key, header_value)
        if key not in by_key:
            by_key[key] = {'header_key': header_key, 'header_value': header_value, 'count': 0, 'emails': [], 'events': []}
        dst = by_key[key]
...
def _request_filters(request):
    ...
    return {'job_id': request.headers.get('x-job-id', '').strip() or q.get('job_id', '').strip() or q.get('x_job_id', '').strip(), 'campaign_id': request.headers.get('x-campaign-id', '').strip() or q.get('campaign_id', '').strip() or q.get('x_campaign_id', '').strip(), 'message_id': request.headers.get('message-id', '').strip() or request.headers.get('x-message-id', '').strip() or q.get('message_id', '').strip() or q.get('msgid', '').strip()}
...
def _recent_matching_files(patterns):
    ...
    for p in PMTA_LOG_DIR.iterdir():
        ...
        if st.st_mtime < min_mtime:
            continue
        files.append({'path': str(p.resolve()), 'name': p.name, 'inode': int(st.st_ino), 'size': int(st.st_size), 'mtime': float(st.st_mtime)})
    if not files:
        raise HTTPException(status_code=404, detail='No accounting/log files matched')
    files.sort(key=lambda x: (x['mtime'], x['name']))
    return files[-max(1, RECENT_PULL_MAX_FILES):]

def _read_from_cursor(files, cursor_payload, limit):
    items = []
    parsed = 0
    skipped = 0
    unknown_outcome = 0
    ...
    if start_idx >= len(files):
        tail = files[-1]
        return {'items': [], 'next_cursor': _encode_cursor({'v': 1, 'path': tail['path'], 'inode': int(tail['inode']), 'offset': int(tail['size']), 'mtime': float(tail['mtime'])}), 'has_more': False, 'stats': {'parsed': 0, 'skipped': 0, 'unknown_outcome': 0}}
    idx = start_idx
    ...
    next_cursor = _encode_cursor({'v': 1, 'path': cursor_file['path'], 'inode': int(cursor_file['inode']), 'offset': int(current_off), 'mtime': float(cursor_file['mtime'])})
    has_more = False
    if consumed_up_to_idx < len(files):
        if int(files[consumed_up_to_idx]['size']) > int(current_off):
            ...
            has_more = True
    return {'items': items, 'next_cursor': next_cursor, 'has_more': has_more, 'stats': {'parsed': parsed, 'skipped': skipped, 'unknown_outcome': unknown_outcome}}

@app.get('/health')
def health():
    return {'ok': True, 'dir': str(PMTA_LOG_DIR), 'server_time_utc': _utc_iso_now()}

@app.get('/')
def root():
    return {'ok': True, 'service': 'PMTA Accounting/Logs API', 'version': '1.1.0', 'endpoints': {'health': '/health', 'files': '/api/v1/files?kind=acct', 'pull': '/api/v1/pull?kind=acct', 'pull_latest': '/api/v1/pull/latest?kind=acct', 'job_outcomes': '/api/v1/job/outcomes?job_id=<job_id>', 'job_count': '/api/v1/job/count?job_id=<job_id>'}}

@app.get('/api/v1/status')
def bridge_status(_=Depends(require_token)):
    with _BRIDGE_STATUS_LOCK:
        state = dict(_BRIDGE_STATUS)
    state['server_time'] = _utc_iso_now()
    resp = {'ok': True}
    resp.update(state)
    return resp

@app.get('/api/v1/job/outcomes')
def get_job_outcomes(job_id='', _=Depends(require_token)):
    ...
    return {'ok': True, 'job_id': jid, 'count': summary['total_unique'], 'linked_emails_count': summary['total_unique'], 'emails': summary['all_emails'], 'delivered': {'count': len(summary['buckets']['delivered']), 'emails': summary['buckets']['delivered']}, 'deferred': {'count': len(summary['buckets']['deferred']), 'emails': summary['buckets']['deferred']}, 'bounced': {'count': len(summary['buckets']['bounced']), 'emails': summary['buckets']['bounced']}, 'complained': {'count': len(summary['buckets']['complained']), 'emails': summary['buckets']['complained']}}

def _calculate_job_outcomes(jid):
    ...
    for ev in _walk_accounting_events(patterns):
        ...
        if ev_jid != jid:
            continue
        rcpt = str(ev.get('rcpt') or ev.get('recipient') or ev.get('email') or ev.get('to') or ev.get('rcpt_to') or '').strip().lower()
        if not rcpt:
            continue
        typ = _normalized_outcome(ev)
        if typ not in {'delivered', 'bounced', 'deferred', 'complained'}:
            print('[pmta-accounting-bridge] unknown outcome job_id={} rcpt={} type={} dsnAction={} dsnStatus={} source_file={} line={}'.format(ev_jid, rcpt, ev.get('type'), ev.get('dsnAction') or ev.get('dsn_action'), ev.get('dsnStatus') or ev.get('dsn_status'), ev.get('source_file'), ev.get('line_no_or_offset')))
            continue
        item = {'email': rcpt, 'outcome': typ, 'message_id': _event_value(ev, 'msgid', 'message-id', 'message_id', 'messageid'), 'dsn_status': _event_value(ev, 'dsnStatus', 'dsn_status', 'enhanced-status', 'enhanced_status'), 'dsn_diag': _event_value(ev, 'dsnDiag', 'dsn_diag', 'diag', 'diagnostic', 'smtp-diagnostic'), 'response': _event_value(ev, 'response', 'smtp-response', 'smtp_response')}
        ...
        if status_rank.get(typ, 0) >= status_rank.get(prev_typ, 0):
            by_recipient[rcpt] = item
    buckets = {'delivered': [], 'deferred': [], 'bounced': [], 'complained': []}
    ...
    for email in sorted(by_recipient.keys()):
        ...
        all_rows.append({'email': email, 'outcome': row.get('outcome') or '', 'message_id': row.get('message_id') or '', 'dsn_status': row.get('dsn_status') or '', 'dsn_diag': row.get('dsn_diag') or '', 'response': row.get('response') or ''})
    total_unique = sum((len(v) for v in buckets.values()))
    return {'total_unique': total_unique, 'all_emails': all_rows, 'buckets': buckets}

@app.get('/api/v1/job/count')
def get_job_count(job_id='', _=Depends(require_token)):
    ...
    return {'ok': True, 'job_id': jid, 'linked_emails_count': summary['total_unique'], 'delivered_count': len(buckets['delivered']), 'deferred_count': len(buckets['deferred']), 'bounced_count': len(buckets['bounced']), 'complained_count': len(buckets['complained'])}

@app.get('/api/v1/files')
def get_files(kind='acct', sort='mtime', order='desc', limit=200, offset=0, _=Depends(require_token)):
    patterns = ALLOWED_KINDS.get(kind)
    if not patterns:
        raise HTTPException(status_code=400, detail='Invalid kind. Use one of: {}'.format(list(ALLOWED_KINDS.keys())))
    ...
    return {'ok': True, 'dir': str(PMTA_LOG_DIR), 'kind': kind, 'total': total, 'count': len(items), 'items': items}

def _pull_accounting(request, kind='acct', max_lines=DEFAULT_PUSH_MAX_LINES, group_by_header=1):
    ...
    if not jid:
        ...
        return {'ok': True, 'job_id': '', 'count': min(len(events), safe_max), 'events': events[-safe_max:], 'source_file': latest_fp.name}
    files = _find_matching_files(patterns)
    events = []
    for fp in files:
        with fp.open('r', encoding='utf-8', errors='replace') as f:
        ...
        if len(events) >= safe_max:
            break
    return {'ok': True, 'job_id': jid, 'count': len(events), 'events': events}

@app.get('/api/v1/pull')
def pull_accounting(request, kinds='acct', cursor='', limit=DEFAULT_PULL_LIMIT, kind='', max_lines=DEFAULT_PUSH_MAX_LINES, group_by_header=1, _=Depends(require_token)):
    ...
    try:
        ...
        for k in requested_kinds:
            ...
            if not p:
                raise HTTPException(status_code=400, detail='Invalid kind: {}. Use one of: {}'.format(k, list(ALLOWED_KINDS.keys())))
            patterns.extend(p)
        ...
        _status_update(last_processed_file=last_processed_file, last_cursor=str(result.get('next_cursor') or ''), parsed=int(stats.get('parsed') or 0), skipped=int(stats.get('skipped') or 0), unknown_outcome=int(stats.get('unknown_outcome') or 0), last_error='')
        resp = {'ok': True, 'kinds': requested_kinds, 'count': len(result['items'])}
        resp.update(result)
        return resp
...
def pull_latest_accounting(request, kind='acct', max_lines=DEFAULT_PUSH_MAX_LINES, group_by_header=1, _=Depends(require_token)):
